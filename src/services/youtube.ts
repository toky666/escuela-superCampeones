...
export const fetchLatestYouTubeVideo =
  async (): Promise<YouTubeVideo | null> => {
    ...
    try {
      ...
      if (searchResponse.status === 403 || searchResponse.status === 500) {
        console.warn(
          `YouTube API error ${searchResponse.status}, using fallback video`,
        );
...
const fallbackVideoId = "-vt-oc1zbEY";
...
const getFallbackVideo = (): YouTubeVideo => {
  return {
    ...
    thumbnail: `https://youtube.com/vi/${fallbackVideoId}/`,
